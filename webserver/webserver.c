...
#include "soapH.h"
//#include "query.nsmap"	/* namespaces updated 4/4/13 */
#include "options.h"
#include "httpget.h"
#include "httpform.h"
#include "logging.h"
...
static long long get_timestamp()
{
	struct timeval t;
	gettimeofday(&t, NULL);
	long long time_stamp_with_msec = (long long) t.tv_sec * 1000;
	time_stamp_with_msec += (long long) t.tv_usec / 1000;
	return time_stamp_with_msec;
}
    ...
    */
        ...
        if (soap_serve(soap) && (soap->error != SOAP_EOF || (soap->errnum != 0 && !(soap->omode & SOAP_IO_KEEPALIVE))))
	{
	  fprintf(stderr, "Request #%d completed with failure %d\n", req, soap->error);
        ...
        printf("point 2. Whole duration = %lld\n", duration);
        soap_destroy(soap);
        soap_end(soap);
      }
    }
...
void *process_request(void *soap)
{
  ...
  long long end = get_timestamp();
  long long duration = end - begin;
  printf("point 3. Whole duration = %lld\n", duration);
  soap_destroy(tsoap);
  soap_end(tsoap);
  soap_done(tsoap);
  free(soap);
...
void *process_queue(void *soap)
{
  ...
  for (;;)
  {
    ...
    long long end = get_timestamp();
    long long duration = end - begin;
    printf("point 1. Whole duration = %lld\n", duration);
...
//specific handling defined by user in *.c
